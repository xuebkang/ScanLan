...
@property (nonatomic, assign) NSInteger timerIterationNumber;
...
- (void)pingAddress{
    ...
    NSString *address = [NSString stringWithFormat:@"%@%ld", self.baseAddress, self.currentHostAddress];
    [SimplePingHelper ping:address target:self sel:@selector(pingResult:)];
    if (self.currentHostAddress>=60) {
        [self.timer invalidate];
    }
}
/*
...
- (void)pingResult:(NSNumber*)success {
    ...
    if (self.timerIterationNumber + self.baseAddressEnd>=60) {
